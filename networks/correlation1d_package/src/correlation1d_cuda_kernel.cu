...
#include "correlation1d_cuda_kernel.h"
                                     ...
                                     int stride2)
{
    ...
    int y1 = blockIdx.y * stride1 + kernel_rad;
        ...
        for (int j = -kernel_rad; j <= kernel_rad; ++j) {
          for (int i = -kernel_rad; i <= kernel_rad; ++i) {
            for (int ch = c; ch < pdimc; ch += THREADS_PER_BLOCK) {
              ...
              // int indx1 = n * pdimyxc + (y1) * pdimxc + (x1 + i) * pdimc + ch;
              int indx2 = n * pdimyxc + (y2+j) * pdimxc + (x2 + i) * pdimc + ch;
              // int indx2 = n * pdimyxc + (y2) * pdimxc + (x2 + i) * pdimc + ch;
...
__global__ void Correlation1D_backward_input1(int item, float *gradInput1, int nInputChannels, int inputHeight, int inputWidth,
                                            ...
                                            int stride2)
  {
    ...
    for (int tc = tch_off; tc < nOutputChannels; tc += THREADS_PER_BLOCK) {
      ...
      int indx2 = n * pdimyxc + (y)* pdimxc + (x + i2) * pdimc + c;
      ...
      for (int j = ymin; j <= ymax; ++j) {
        for (int i = xmin; i <= xmax; ++i) {
          ...
          // int tindx = n * tdimcyx + tc * tdimyx + y * tdimx + i;
...
__global__ void Correlation1D_backward_input2(int item, float *gradInput2, int nInputChannels, int inputHeight, int inputWidth,
                                            float *gradOutput, int nOutputChannels, int outputHeight, int outputWidth,
                                            float *rInput1,
                                            int pad_size,
                                            int kernel_size,
                                            ...
                                            int stride2)
{
    ...
    for (int tc = tch_off; tc < nOutputChannels; tc += THREADS_PER_BLOCK) {
      ...
      int indx1 = n * pdimyxc + (y)* pdimxc + (x - i2) * pdimc + c;
